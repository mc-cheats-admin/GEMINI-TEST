            ...
            return (
              ...
              >
                ...
                <div className="w-full pl-12 md:pl-0 md:w-5/12">
                  <div 
                    className={`glass-panel p-6 rounded-2xl transition-all duration-700 ${isActive ? 'opacity-100 translate-x-0' : `opacity-0 ${isLeft ? 'md:-translate-x-20' : 'md:translate-x-20'} translate-x-10`}`}
                  >
                    <div className="font-mono text-xl md:text-2xl text-[var(--color-neon-green)] mb-2 drop-shadow-[0_0_5px_rgba(0,255,0,0.5)]">
                      &gt; {event.year}_
                    </div>
                    <h3 className="text-xl md:text-2xl font-display font-bold text-white mb-3">{event.title}</h3>
                    <p className="text-white/70 font-sans font-light leading-relaxed">{event.desc}</p>
                  </div>
                </div>
