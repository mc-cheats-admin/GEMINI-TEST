  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[var(--color-dark-bg)]/80 backdrop-blur-md py-4 border-b border-white/10' : 'bg-transparent py-6'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        <div className="interactive font-display text-2xl font-bold tracking-widest text-white flex group cursor-pointer">
        ...
        <div className="hidden md:flex items-center space-x-8">
          {navLinks.map((link, i) => (
            <a key={i} href={`#${link.toLowerCase()}`} className="interactive relative font-sans text-sm tracking-widest text-white/80 hover:text-white transition-colors group">
              {link}
          ...
          <button onClick={toggleTheme} className="interactive text-white/80 hover:text-white transition-colors">
            {theme === 'dark' ? <Sun size={20} /> : <Moon size={20} />}
          </button>
        </div>

        <button className="interactive md:hidden text-white z-50" onClick={() => setMenuOpen(!menuOpen)}>
        ...
        <div className="flex flex-col items-center space-y-8">
          {navLinks.map((link, i) => (
            <a 
              ...
              className="interactive font-display text-2xl tracking-widest text-white hover:text-[var(--color-neon-blue)] transition-colors"
          ...
          <button onClick={() => { toggleTheme(); setMenuOpen(false); }} className="interactive text-white mt-8 flex items-center space-x-2">
