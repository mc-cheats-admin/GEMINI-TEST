    ...
    <section id="возможности" className="py-32 relative z-10" ref={ref}>
      <div className="container mx-auto px-6">
        <div className="text-center mb-20">
          ...
          <p className={`typewriter text-lg text-white/70 max-w-2xl mx-auto font-sans font-light ${hasIntersected ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div 
              key={index} 
              className={`glass-panel p-8 rounded-2xl relative overflow-hidden group transition-all duration-700 delay-${index * 100} ${hasIntersected ? 'opacity-100 translate-y-0 rotate-0' : 'opacity-0 translate-y-20 rotate-6'}`}
            ...
            >
              ...
              <h3 className="text-2xl font-display font-semibold mb-4 tracking-wide text-white group-hover:text-[var(--color-neon-blue)] transition-colors duration-300">
                {feature.title}
              </h3>
              
              <p className="text-white/60 font-sans font-light leading-relaxed group-hover:text-white/80 transition-colors duration-300">
                {feature.desc}
              </p>
